...
def dashboard(request):
    ...
    if request.method == 'POST':
        if 'document' in request.FILES:
            ...
            return redirect('app-dashboard', document_id=document.id)
    ...
    if request.method == 'POST' and request.FILES.get('pdf'):
        ...
        if os.path.exists(pdf_path):
            output = ask_pdf_groq(pdf_path, marking_prompt)  # Custom function to mark assignments
        else:
    ...
    return render(request, 'app/dashboard.html', {'output': output, 'filename': filename, 'documents': documents})
